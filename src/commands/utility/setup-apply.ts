...
const prefix = 'cyberbot:setup-apply';

export default {
  metadata: {
    name: 'setup-apply',
    description: 'Kurulumu uygular.',
    category: 'utility',
    cooldown: 60,
  ...
  data: new SlashCommandBuilder()
    .setName('setup-apply')
    .setDescription('Kurulumu uygular.')
  ...
  async execute({ interaction, member, guild }: CommandContext) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`# Kurulum Onayı\n\n**${profileData.displayName}** profili ile kurulum yapılacak. Devam etmek istediğinize emin misiniz?`),
    );
...
export async function handleSetupApplyComponent(
